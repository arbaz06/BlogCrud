    ...
    <div class="form-group ml-auto">
        <form action="{{ route('blog-posts.update-status', $post->id) }}" method="POST">
            ...
            <div class="form-check form-switch">
                <input class="form-check-input" type="checkbox" name="status" onchange="this.form.submit()" {{ $post->status ? 'checked' : '' }}>
                <label class="form-check-label" for="status">{{ $post->status ? 'Active' : 'Inactive' }}</label>
            </div>
        </form>
    </div>
</td>
